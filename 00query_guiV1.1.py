...
# 每页条数选项
PAGE_SIZES = [10, 20, 50, 100, 500, 1000, 2000, 10000]
...
class QueryPanel(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Label(bar, text="关键词:", font=(DEFAULT_FONT, 10)) \
        ...
        if self.query_type in ("site", "app", "code"):
            ttk.Label(bar, text="排序:", font=(DEFAULT_FONT, 9)) \
        ...
        ttk.Label(self, text=hint_text, font=(DEFAULT_FONT, 8),
        ...
        ttk.Label(self, textvariable=self.info_var, font=(DEFAULT_FONT, 9),
        ...
        ttk.Label(pag, text="每页:", font=(DEFAULT_FONT, 9)) \
        ...
        ttk.Label(pag, text=" 第", font=(DEFAULT_FONT, 9)) \
           .pack(side=LEFT)
        ...
        ttk.Label(pag, text="页 ", font=(DEFAULT_FONT, 9)) \
           .pack(side=LEFT)
        ...
        ttk.Label(pag, textvariable=self.total_var, font=(DEFAULT_FONT, 9),
    ...
    def _bind_events(self):
        ...
        self.tv.bind("<Button-3>", self._on_right_click)  # 右键菜单
    ...
    def _insert_row(self, row: dict):
        display_row = self._get_display_row(row)
        vals = [display_row.get(c[0], "") for c in self._columns]
    ...
    def _show_suspected_filters(self, result: dict):
        ...
        ttk.Label(c, text="📋 疑似信息系统 — 高级查询语法",
                  font=(DEFAULT_FONT, 13, "bold")).pack(anchor=W, pady=(0, 5))
        ttk.Label(c, text="以下查询语法由系统自动生成，可用于进一步搜索关联目标：",
                  font=(DEFAULT_FONT, 9), foreground="gray") \
    ...
    def _export_xlsx(self, path: str):
        ...
        # 表头样式
        hdr_font = Font(name=DEFAULT_FONT, bold=True, color="FFFFFF", size=10)
        ...
        cell_font = Font(name=DEFAULT_FONT, size=9)
    ...
    def _on_right_click(self, event):
        ...
        row_data = self._get_display_row(self._data[idx])
        menu = tk.Menu(self, tearoff=0, bg="#ffffff", fg="#333333",
                       activebackground="#4A90D9", activeforeground="white",
                       font=(DEFAULT_FONT, 9))
        menu.add_command(label=f"📋 复制整行 (CSV)", font=(DEFAULT_FONT, 9),
                         command=lambda: self._copy_row_csv(idx))
        menu.add_separator()
        for field_key, field_label, _ in self._columns:
            val = row_data.get(field_key, "")
            display = str(val)[:40] + ("..." if len(str(val)) > 40 else "")
            label = f"复制 {field_label}  →  {display}"
            menu.add_command(
                label=label,
                font=(DEFAULT_FONT, 9),
        ...
        menu.add_separator()
        menu.add_command(label="📄 查看详情", font=(DEFAULT_FONT, 9),
    ...
    def _copy_val(self, val):
        """复制到剪贴板（复用当前窗口，避免创建临时 Tk 实例）"""
        self.clipboard_clear()
        self.clipboard_append(str(val) if val else "")
        self.update()

    def _copy_row_csv(self, idx: int):
        if idx >= len(self._data):
            return
        row_data = self._get_display_row(self._data[idx])
        parts = [str(row_data.get(c[0], "")) for c in self._columns]
        text = ",".join(parts)
        self._copy_val(text)
...
class KeyConfigDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        ttk.Label(outer,
                  ...
                  font=(DEFAULT_FONT, 8), foreground="#6c757d") \
            .grid(row=row, column=0, columnspan=2, sticky="w",
                  padx=32, pady=(4, 20))
        row += 1
...
class SyntaxGuideWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        ttk.Label(outer, text="📖 0.zone 查询语法参考指南",
                  font=(DEFAULT_FONT, 15, "bold")).pack(anchor=W, pady=(0, 2))
        ...
        ttk.Label(bf, text="💡 提示: 双击任意行可复制语法  |  Esc 关闭",
                  font=(DEFAULT_FONT, 8), foreground="gray").pack(side=LEFT)
    ...
    def _build_base_tab(self, parent):
        ...
        ttk.Label(inner, text="📝 查询条件书写规范",
                  font=(DEFAULT_FONT, 12, "bold")).pack(anchor=W, pady=(10, 5))
        rules = [
        ...
        for r in rules:
            ttk.Label(inner, text=r, font=(DEFAULT_FONT, 9),
                      foreground="#333", wraplength=860, justify=LEFT) \
               .pack(anchor=W, padx=10, pady=1)

        # ── 逻辑连接符 ──
        ttk.Label(inner, text="🔗 逻辑连接符",
                  font=(DEFAULT_FONT, 12, "bold")).pack(anchor=W, pady=(15, 5))
        logic_data = [
    ...
    def _build_module_tab(self, parent, mod_key: str):
        ...
        ttk.Label(inner, text=f"🔍 {self._mod_label(mod_key)} — 高级查询参数",
                  font=(DEFAULT_FONT, 12, "bold")).pack(anchor=W, pady=(10, 8))
        ...
        if params:
            ...
            ttk.Label(inner, text="(暂无参数数据)", font=(DEFAULT_FONT, 9)) \
...
class MainApp(ttk.Window):
    ...
    def _build_ui(self):
        ...
        tt = ttk.Frame(tf)
        tt.pack(side=LEFT)
        ttk.Label(tt, text=APP_NAME, font=(DEFAULT_FONT, 17, "bold")).pack(anchor=W)
        ttk.Label(tt, text="基于 0.zone API · 企业资产信息综合查询平台 作者：地图大师和 AI",
                  font=(DEFAULT_FONT, 9), foreground="gray").pack(anchor=W)
        ...
        ttk.Label(kf, textvariable=self.key_status_var,
                  font=(DEFAULT_FONT, 9)).pack(side=LEFT, padx=(0, 10))
        ...
        # 移动端应用默认搜索语法
        self.panels["app"].query_var.set(
            "company==请替换为公司名&&(type==微信公众号||type==微信小程序)"
        )
        ...
        ttk.Label(ft, textvariable=self.status_var,
                  font=(DEFAULT_FONT, 8), foreground="gray").pack(side=RIGHT)
